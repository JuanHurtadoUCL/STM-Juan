...
bt_do =uicontrol(fig_choose,'style','pushbutton','units','normalized','Position',[0.5320 0.0110 0.4192 0.1027],'String','Do Analysis','Callback',@fun_do);
    function fun_do(~,~)
        ...
        %close(fig_choose)
        doanalysis(case_program,Rs_calculated,cse,freq,cal_trace,Temp_tip,Temp_substrate);
    end
disp('f')
    ...
    function doanalysis(caso,Series_Resistor,Series_Capacitor,Frequency,Trace_Calibration,Temp_tip,Temp_sub)
        ...
        switch caso
            case 1 % Calibration
                ...
                piezo_volt = piezo_raw./20;                                % Voltage divisor at the Adwin output 1/50
                ...
                figure
                plot(piezo_volt,G_dc,'k');
                hold on
                plot(piezo_volt,G_ac,'r','linewidth',1);
                legend('DC','AC')
...
%% Functions
    ...
    function [g_dc] = cunductancedc(traces_dc,rserie_fun,freq_dc,piezo_v)
        ...
        vdc_sat=mean(vdc_zer(cuta:2*cuta));
