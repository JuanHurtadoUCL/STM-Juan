% Code Open Files Low T

selpath = uigetdir;
save_filename = ['RoomT_STM_psPara_factor.mat'];  % This is the name of the saved file
cd(selpath)
Rg=1e8;                              % Gain resistance
Vb=0.099;                            % Vbias
factor_piezo=35*0.15;                % Volts to nm   * divisor
%num_trace=[4316 21158];             % Number of the traces. First one and Last One
curva=[];
ind=0;
G_tot=[];
Z_tot=[];
...
disp(['There are ' num2str(length(folder_trace)) ' traces in this folder']);
%%
...
function [G,V_corr,rserie] = fromvtog(piezo_v,data_v,resistor,bias)
...
data_zer=data_v-mean(data_v(end-50:end));
sat=mean(data_zer(1:50));
...
G = Gj./G0;
lG = smooth(log10(G));
zer_cor = mean(piezo_v(lG>-0.8 & lG<-0.3));

V_corr = (piezo_v-zer_cor); 
